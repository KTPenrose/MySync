...
pub fn parse_args(args: impl Iterator<Item = String>) -> UserInstructions {
    ...
    for mut arg in args {
        ...
        if arg == "help" || arg == "-h" {
            ...
            if !user_instructions.action.is_none() {
                user_instructions.error_messages.push(format!(
                    "Two actions defined: '{}' and 'clone'",
                    user_instructions.action
                ));
            } else {
            ...
            if !matches!(user_instructions.sync_strategy, SyncStrategy::Sync) {
                user_instructions.error_messages.push(format!(
                    "Can't use the -i flag with sync strategy '{}'",
                    user_instructions.sync_strategy
                ));
            } else if !matches!(user_instructions.action, ActionTypes::Sync) {
                user_instructions.error_messages.push(format!(
                    "Can't use the -i flag with action '{}'",
                    user_instructions.action
                ));
            } else {
            ...
            if user_instructions.is_interactive {
                user_instructions
                    .error_messages
                    .push(format!("Can't use the -i flag with sync strategy 'Push'"));
            } else {
            ...
            if user_instructions.is_interactive {
                user_instructions
                    .error_messages
                    .push(format!("Can't use the -i flag with sync strategy 'Pull'"));
            } else {
            ...
            if !matches!(user_instructions.action, ActionTypes::Sync) {
                user_instructions.error_messages.push(format!(
                    "Can't use the -checksum flag with action '{}'",
                    user_instructions.action
                ));
            } else {
            ...
            if user_instructions.origin.is_some() {
                ...
                user_instructions.error_messages.push(format!(
                    "Is '{}' an origin? I was already using '{}' as the origin...",
                    arg,
                    origin.unwrap()
                ));
                ...
                user_instructions
                    .error_messages
                    .push(format!("Unexepected argument: {}", arg));
            } else {
    ...
    if user_instructions.action.is_none() {
        ...
        user_instructions
            .error_messages
            .push(format!("No action specified."));
    } else if matches!(user_instructions.action, ActionTypes::Clone) {
        if user_instructions.origin.is_none() {
            user_instructions.is_help = true;
            user_instructions
                .error_messages
                .push(format!("No origin was specified."));
        }
    }
...
mod test {
    ...
    fn test_clone_with_origin() {
        ...
        matchui(
            ...
            UserInstructions {
                is_help: false,
                action: ActionTypes::Clone,
                is_interactive: false,
                sync_strategy: SyncStrategy::Sync,
                do_checksum: false,
                force_lock: false,
                origin: Option::Some("c:\\mydir".to_string()),
                error_messages: Vec::new(),
            },
        );
    }
    ...
    fn test_sync_pull_checksum() {
        let args = std::array::IntoIter::new([
            "sync".to_string(),
            "-pull".to_string(),
            "-checksum".to_string(),
        ]);
    ...
    fn test_clone_no_origin_interactive() {
        ...
        assert_eq!(
            "Can't use the -i flag with action 'Clone'",
            user_instructions.error_messages[0]
        );
        assert_eq!(
            "No origin was specified.",
            user_instructions.error_messages[1]
        );
    }
    ...
    fn test_sync_interactive_push() {
        let args =
            std::array::IntoIter::new(["sync".to_string(), "-i".to_string(), "-push".to_string()]);
        let user_instructions: UserInstructions = crate::my_sync_arg_parser::parse_args(args);
        assert!(user_instructions.is_help);
        assert_eq!(user_instructions.error_messages.len(), 1);
        assert_eq!(
            "Can't use the -i flag with sync strategy 'Push'",
            user_instructions.error_messages[0]
        );
    }
    ...
    fn test_sync_push_interactive() {
        let args =
            std::array::IntoIter::new(["sync".to_string(), "-push".to_string(), "-i".to_string()]);
        let user_instructions: UserInstructions = crate::my_sync_arg_parser::parse_args(args);
        assert!(user_instructions.is_help);
        assert_eq!(user_instructions.error_messages.len(), 1);
        assert_eq!(
    ...
    fn test_sync_pull_interactive() {
        let args =
            std::array::IntoIter::new(["sync".to_string(), "-pull".to_string(), "-i".to_string()]);
        let user_instructions: UserInstructions = crate::my_sync_arg_parser::parse_args(args);
        assert!(user_instructions.is_help);
        ...
        assert_eq!(user_instructions.error_messages.len(), 1);
        assert_eq!(
            "Can't use the -i flag with sync strategy 'Pull'",
            user_instructions.error_messages[0]
        );
    }
    ...
    fn test_sync_interactive_pull() {
        let args =
            std::array::IntoIter::new(["sync".to_string(), "-i".to_string(), "-pull".to_string()]);
        let user_instructions: UserInstructions = crate::my_sync_arg_parser::parse_args(args);
        assert!(user_instructions.is_help);
        for msg in &user_instructions.error_messages {
            println!("error: {}", &msg);
